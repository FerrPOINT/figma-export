...
figma.ui.onmessage = async (msg) => {
  switch (msg.type) {
    ...
    case "execute-command":
      ...
      console.log("Plugin received command:", msg.command, "with params:", msg.params);
      try {
        const result = await handleCommand(msg.command, msg.params);
        console.log("Command executed successfully, sending result back");
...
async function handleCommand(command, params) {
  console.log(`🔍 DEBUG: handleCommand called with command: "${command}"`);
  ...
  switch (command) {
    ...
    case "export_all_nodes":
      return await exportAllNodesWithStackAlgorithm(params);
    default:
...
async function getNodesInfo(nodeIds) {
  try {
    ...
    sendProgressUpdate(
      ...
      nodeIds.length,
      0,
      `Starting export of ${nodeIds.length} nodes`
    );
    ...
    for (let i = 0; i < nodeIds.length; i++) {
      try {
        ...
        // Get basic node info (without heavy exportAsync)
        const nodeInfo = {
          nodeId: node.id,
          name: node.name,
          type: node.type,
        ...
        sendProgressUpdate(
          ...
          totalCount,
          processedCount,
          `Processed node ${processedCount}/${totalCount}: ${node.name}`
        );
        
      } catch (nodeError) {
        console.error(`Error processing node ${nodeIds[i]}: ${nodeError.message}`);
        processedCount++;
        sendProgressUpdate(
          commandId,
          "get_nodes_info",
          ...
          totalCount,
          processedCount,
          `Error processing node: ${nodeIds[i]}`
        );
      }
    }
    ...
    sendProgressUpdate(
      ...
      totalCount,
      totalCount,
      `Completed export of ${totalCount} nodes`
    );
...
async function readMyDesign() {
  try {
    // Если есть выделенные элементы, используем их
    if (figma.currentPage.selection.length > 0) {
      ...
      // Если ничего не выделено, экспортируем весь документ
      const document = await figma.getDocumentDataAsync();
      return {
...
async function exportNodeAsImage(params) {
  const { 
    nodeId, 
    scale = 4, // Увеличиваем масштаб по умолчанию с 1 до 4
    format = "PNG",
    constraint = "SCALE" // Добавляем поддержку разных типов ограничений
  } = params || {};
  ...
  try {
    // Улучшенные настройки экспорта для максимального качества
    const settings = {
      format: format,
      constraint: { 
        type: constraint, 
...
async function scanNodesByTypes(params) {
  console.log(`🔍 DEBUG: scanNodesByTypes called with params:`, JSON.stringify(params, null, 2));
...
// Export all nodes function
async function exportAllNodesWithStackAlgorithm(params) {
  console.log(`🔍 DEBUG: exportAllNodesWithStackAlgorithm called with params:`, JSON.stringify(params, null, 2));
  const commandId = params?.commandId || generateCommandId();
  const maxDepth = params?.maxDepth || 100;
      const batchSize = params?.batchSize || 5;
  
  console.log(`🚀 Starting professional stack-based traversal algorithm`);
  
  try {
    // Step 1: Get document info
    console.log(`📋 Step 1: Getting document info...`);
    const documentInfo = await getDocumentInfo();
    
    // Step 2: Get all pages
    console.log(`📄 Step 2: Getting all pages...`);
    const allPages = await readMyDesign();
    
    // Step 3: Get all styles
    console.log(`🎨 Step 3: Getting all styles...`);
    const allStyles = await getStyles();
    
    // Step 4: Get all components
    console.log(`🧩 Step 4: Getting all components...`);
    const allComponents = await getLocalComponents();
    
    // Step 5: Professional Stack-Based Traversal Algorithm
    console.log(`🔍 Step 5: Starting stack-based traversal...`);
    
    const allNodes = [];
    const processedNodes = new Set();
    const frameCount = { total: 0, processed: 0 };
    
    // Stack-based traversal function
    function traverseWithStack(rootNodes) {
      const stack = [];
      const nodeStack = [];
      ...
      console.log(`📦 Stack initialized with ${stack.length} root nodes`);
      
      // Process stack until empty
      while (stack.length > 0) {
        const current = stack.pop();
        const { node, depth, parentPath, parentId } = current;
        
        // Skip if already processed or depth exceeded
        if (depth > maxDepth || !node || processedNodes.has(node.id)) {
          continue;
        }
        ...
        if (allNodes.length % 100 === 0) {
          console.log(`📊 Processed ${allNodes.length} nodes, stack size: ${stack.length}`);
        }
      }
    }
    
    // Start traversal from all pages
    if (allPages && allPages.children && Array.isArray(allPages.children)) {
      traverseWithStack(allPages.children);
    }
    
    console.log(`📊 Stack traversal completed: ${allNodes.length} nodes found (${frameCount.total} frames)`);
    
    // Step 6: Process nodes in batches using get_node_info
    console.log(`🔄 Step 6: Processing nodes with detailed info...`);
    const results = [];
    ...
    for (let i = 0; i < totalNodes; i += batchSize) {
      ...
      console.log(`🔄 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(totalNodes / batchSize)}`);
      
      // Process each node using get_node_info
      for (const nodeInfo of batch) {
        try {
          const nodeData = await getNodeInfo(nodeInfo.id);
      ...
      // Update progress
      const processedCount = Math.min(i + batchSize, totalNodes);
      sendProgressUpdate(
        commandId,
        "export_all_nodes",
        "in_progress",
        ...
        totalNodes,
        processedCount,
        `Exported ${processedCount}/${totalNodes} nodes`
      );
    ...
    // Step 7: Get annotations
    console.log(`📝 Step 7: Getting annotations...`);
    const annotations = await getAnnotations({});
    
    // Step 8: Get reactions
    console.log(`🎭 Step 8: Getting reactions...`);
    const nodeIdsForReactions = allNodes.map(n => n.id);
    const reactions = await getReactions(nodeIdsForReactions);
    
    // Step 9: Build hierarchical structure
    console.log(`🏗️ Step 9: Building hierarchical structure...`);
    ...
    sendProgressUpdate(
      ...
      totalNodes,
      totalNodes,
      `Successfully exported ${results.length} nodes with stack algorithm`
    );
    
    console.log(`✅ Professional stack-based export completed: ${results.length} nodes processed`);
    
    return {
      success: true,
      totalNodes: totalNodes,
      exportedNodes: results.length,
      totalFrames: frameCount.total,
      processedFrames: frameCount.processed,
      algorithm: "Stack-Based Traversal",
      exportData: {
        ...
        allNodes: results,
        hierarchicalStructure: rootNodes,
      ...
      exportInfo: {
        ...
        traversalStats: {
          totalNodes: totalNodes,
          totalFrames: frameCount.total,
          maxDepthReached: Math.max(...results.map(n => n.depth)),
          averageDepth: results.reduce((sum, n) => sum + n.depth, 0) / results.length
        }
      }
    };
    
  } catch (error) {
    console.error("❌ Stack-based export failed:", error);
    ...
    sendProgressUpdate(
      ...
      0,
      0,
      `Stack-based export failed: ${error.message}`
    );
